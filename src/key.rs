...
pub fn encode(locker: LockerId, key: &str) -> Vec<u8> {
    ...
    let mut out = Vec::with_capacity(PREFIX_LEN + bytes.len());
...
pub fn prefix_range(locker: LockerId, prefix: &str) -> KeyRange {
    KeyRange::prefix(&encode(locker, prefix))
}
...
pub fn encode_range(locker: LockerId, start: Bound<&str>, end: Bound<&str>) -> KeyRange {
    ...
    let start = match start {
        ...
        Bound::Included(k) => Bound::Included(encode(locker, k)),
        Bound::Excluded(k) => Bound::Excluded(encode(locker, k)),
    };
    let end = match end {
